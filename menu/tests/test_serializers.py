...
from menu.models import Restaurant, Menu, Vote, EmployeeProfile
from menu.serializers import RestaurantSerializer, MenuSerializer, UserSerializer, VoteSerializer, EmployeeProfileSerializer

@pytest.fixture
def restaurant_data():
    return {'id': 5, 'name': 'Test Restaurant'}
...
@pytest.mark.django_db
def test_serialize_employee_profile():
    user = User.objects.create_user(username='testuser', email='test@example.com', password='testpassword')
    employee_data = {
        'id': 1,
        'user': user.pk
    }
    serializer = EmployeeProfileSerializer(data=employee_data)
    ...
    serialized_data = serializer.data
    serialized_data['id'] = instance.id
    assert serialized_data == employee_data
